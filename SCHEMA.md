...
Easter Sunday: `mag:francis-i/urbi-et-orbi-2020-03-27`), marked by `idStatus`. Full rules are in
...
24. **Ordinal is consistent with the series' first year.** When `series.ordinal`, `series.year` and the vocabulary row's
    `firstYear` are all present, `ordinal = year − firstYear + 1`. It never fires when any of the three is absent — an ordinal
