...
25. **One page of the Acta opens one act.** No two documents share `(acta.series, acta.volume, acta.part, acta.page)` — save
    ...
    `tools/src/acta/curation.ts`; *AAS 70 (1978) 150*, two apostolic letters), where exactly the documents the row names may
    share it. Well-formedness of the reference is the schema's; this is the only invariant that reads `acta`, which bears on
