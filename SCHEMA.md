...
**`characteristics`** (non-exclusive document-level metadata a genre row *allows*) — `apostolic-constitution` · `dogmatic-definition` · `in-forma-brevis` · `motu-proprio`. The vocabulary is flat; which genre may bear which is declared per genre in `genre.allowedCharacteristics` (`papal-bull` allows the first two, `apostolic-letter` the last two; a genre row without the field allows none) and enforced by invariant 22. A document may carry none, one, or several of the characteristics its genre allows (e.g. *Munificentissimus Deus* carries both bull characteristics; *Socialium Scientiarum* carries `motu-proprio`). `motu-proprio` records a mode of issuance — the pope acting on his own initiative — not a genre, and makes no authority claim. `in-forma-brevis` likewise records the form of the instrument — a brief, *Litterae Apostolicae in forma Brevis* sealed *sub anulo Piscatoris* — not a genre, and makes no authority claim.
...
**`acta`** (optional object `{ "series", "volume", "year", "page" }`, all four required when present) — where the act stands in the *Acta Apostolicae Sedis* (`series: "AAS"`, 1909–) or, before 1909, the *Acta Sanctae Sedis* (`"ASS"`): the citation of record every apparatus expects, *AAS 115 (2023) 1041* being `{ "series": "AAS", "volume": 115, "year": 2023, "page": 1041 }`. `volume` is the number the index prints (*Vol. CXV*), read, never computed, although for the AAS it is `year − 1908`; `year` is the volume year, which differs from `date` for a December act published in the next year's volume; `page` is the **first page** as the index cites it, no range. An optional `part` (`"I"` / `"II"`) names the part of the two-part volumes of 1917 and 1983, whose pagination restarts per part: *AAS 9-I (1917) 417* is `{ "volume": 9, "part": "I", "page": 417 }` (every 1917 and every 1983 reference carries `"I"`; part II of each is the *Codex Iuris Canonici* of that year and has no chronological index). The reference **joins** the shelf harvest and does not replace it: `npm run harvest` writes it on the documents an entry of the AAS chronological index — the annual *Index generalis* PDFs of 2003–2024 (2003–2009 in phase 2b′, 2010, 2011, 2013 and 2014 in phase 2b-ii-c, 2012 in the sample, 2015–2024 in phase 1), and the index pages of the whole-volume PDFs of 1909, 1917-I, 1931, 1958 and 1978, the sample of phase 2b-i, of 1932–1957, AAS 24–49, phase 2b-ii-a, of 1959–1977, AAS 51–69, phase 2b-ii-b, of 1979–2002, AAS 71–94, phase 2b-ii-c, of 1926–1930, AAS 18–22, phase 2b-iii-a, and of 1909–1925, AAS 1–17, phase 2b-iii-b, whose pages the OCR lost and the volume bodies gave back (`tools/fixtures/acta/` for the fixtures and the sidecars, `tools/src/acta/`, `tools/src/acta/recover.ts` for the recovery), and — before 1909 — the *Acta Sanctae Sedis* of the five sample volumes of phase 2c-i (ASS 1, 12, 23, 33, 41; `docs/superpowers/specs/2026-09-21-ass-volumes-design.md`), whose entries are synthesised from the volume body (`tools/src/acta/ass.ts`, the fixtures `ass-{vol}-{year}.entries.json`, every entry quoting the lines it was read from) since the ASS print no chronological index, and checked for completeness against the volume's own *Summa actorum* (`tools/src/acta/summa.ts`); an ASS `year` is the **first year** of a two-year volume (ASS 33 (1900–01) is `{ "series": "ASS", "volume": 33, "year": 1900 }`) — matches by issuer, date and incipit (for the ASS, whose entries carry no incipit, the shelf's incipit as a word-boundary prefix of the act's opening words, read after the salutation), and reports rather than writes whatever it cannot evidence — an ambiguous entry, an act the shelves lack, a document two entries claim, two matched documents on one page the volume was not read for (the join reports are in `docs/superpowers/reports/`). An act the *Acta* print twice has one citation of record — the first printing, unless the volume marks the later as a correction — decided per act in `ACTA_REPRINTS` (`tools/src/acta/curation.ts`) with the fascicles quoted. A page the index prints wrongly for an act it dates and names (the OCR's `530` for *Casti connubii* at 539) is replaced from `ACTA_PAGE_CORRECTIONS` (same file), both pages read in the volume, before the join matches or creates. For the acts the shelves lack, the reference is also the **source**: a document created from the index (phase 2a, `tools/src/acta/create.ts`) carries `source.shelf` of the form `aas/{volume-year}` and its `acta` says where the act was read. Like `keywords`, `series` and `actKind`, `acta` is **never authority-bearing**: it makes no claim about register, definitiveness or solemn form, and the only invariant that reads it is 25 (uniqueness of `(series, volume, part, page)`, since one page opens one act — except the pages a volume prints two short acts on, curated with the page quoted in `ACTA_SHARED_PAGES`, `tools/src/acta/curation.ts`) beside the schema's well-formedness. See [#25](https://github.com/CatholicOS/cmddr/issues/25).

**`source`** (optional object `{ "url", "shelf", "alsoShelvedAs"?, "languages"?, "retrieved" }`) — where the record was read from, never a claim about the act. For a shelf record: the vatican.va page (`url`), the shelf it was filed on (`shelf`: `apost_letters`, `messages/peace`, …; `null` for a flat-era page) and any other shelf the same act is filed on (`alsoShelvedAs`), the languages the page offers, and the date the fixture was fetched. For a document **created from the *Acta Apostolicae Sedis* index** because the shelves lack the act (the AAS-only documents of phase 2a of [#25](https://github.com/CatholicOS/cmddr/issues/25)): `shelf` is `aas/{volume-year}` (`aas/2023`), `url` is the whole-volume PDF for a volume source of 1909–2002 (`https://www.vatican.va/archive/aas/documents/AAS-23-1931-ocr.pdf`) and `null` in the fascicle era (2003–), since the monthly fascicle holding a page is not derivable from the index, `languages` is omitted, and `retrieved` is the fixture's date (`tools/fixtures/acta/README.md`, `ACTA_SOURCES`). Such a record is created only in a category whose vatican.va shelf is harvested for the pope, after a duplicate guard has found no shelf record it could be — a same-date record carrying its incipit or toponym under another class, a same-date record with no incipit at all, a same-incipit record a day off or anywhere in the pontificate — and every entry the guard or a rule holds is listed with its reason in the join report.
...
25. **One page of the Acta opens one act.** No two documents share `(acta.series, acta.volume, acta.part, acta.page)` — save
    the pages a volume itself prints two short acts on, each curated with the page quoted (`ACTA_SHARED_PAGES`,
    `tools/src/acta/curation.ts`; *AAS 70 (1978) 150*, two apostolic letters; *AAS 24 (1932) 39*, *26 (1934) 19*, *28 (1936)
    102* and *45 (1953) 91*, the volumes of the 1930s and 1950s setting two short letters on one page as a matter of course),
    where exactly the documents the row names may share it; an entry created from the index whose page another act cites is
    held instead (`page-shared`, `create.ts`), since the OCR of a volume misreads page numbers onto another act's. Well-formedness of the reference is the schema's; this is the only invariant that reads `acta`, which bears on
    nothing but the citation — the harvest withholds a reference two index entries claim for one document rather than let
    this rule catch it.
