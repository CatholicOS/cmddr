import { cell, byDateThenId, idCell, actaCell, FOOTNOTE } from './table.js';
...
| ID | Title | Incipit | Issuer | Date | Promulgated by | AAS |${genreId === null ? ' Source label |' : ''}
| --- | --- | --- | --- | --- | --- | --- |${genreId === null ? ' --- |' : ''}`;

  const body = rows.map((d) =>
    `| ${idCell(d)} | ${cell(d.title)} | ${d.incipit ? cell(d.incipit) : ''} `
    + `| \`${d.issuerId}\` | ${d.date} | ${d.promulgatedBy ? `\`${d.promulgatedBy}\`` : ''} | ${actaCell(d)} |`
