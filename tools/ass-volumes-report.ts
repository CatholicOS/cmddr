...
p(`   ${umClassHold.length} ${umClassHold.length === 1 ? 'is' : 'are'} held by the class rule against an encyclical (finding 7), and ${umClaimedElsewhere.length} is the Latin printing whose one candidate the`);
...
p(`7. **One act of weight is held by the class rule, and the other proved to be a filing error.**`);
p(`   *Catholicae Ecclesiae* of 20 November 1890, on slavery in Africa, is headed \`LITTERAE circulares\` at ASS 23 p. 257`);
p(`   and shelved as an encyclical (\`mag:leo-xiii/catholicae-ecclesiae-1890\`). It has **exactly one shelf record of its own**`);
p(`   **incipit on its own date**, printed in §3.3's candidate column, and the join refuses it only because *letter* and`);
p(`   *encyclical* are different classes. What the sources show: the ASS heads an act by what the chancery called the`);
p(`   instrument, the shelf files it by what the act is — and \`LITTERAE circulares\` names what an encyclical is, a circular`);
p(`   letter. What I judge, separately: reading \`LITTERAE CIRCULARES\` as a class heading of its own clears it by rule`);
p(`   rather than by a curated row, and measured over all 41 volumes it moves this one act and nothing else. **Nothing was**`);
p(`   **written here.** The decision is editorial, and it decides more than this act: the ASS calls an encyclical`);
p(`   \`EPISTOLA ENCYCLICA\`, \`LITTERAE ENCYCLICAE\`, \`LETTERA ENCICLICA\` and, twice, plain \`LITTERAE\`, so a ruling`);
p(`   here is a ruling for the ${volumesLeft} volumes 2c-ii will scan.`);
p(`   *Omnibus compertum* of 21 July 1900, to the Melkites, stood beside it until this phase. It is headed \`LITTERAE\` at`);
p(`   ASS 33 p. 65, and its shelf record carried \`encyclical\` only because vatican.va files the act under encyclicals in`);
p(`   **English alone**, while the Latin and the Italian sit on the letters shelf, headed`);
p(`   \`LEONE XIII EPISTOLA OMNIBUS COMPERTUM\` — the shelf of a translation deciding the genre. A \`GENRE_OVERRIDES\``);
p(`   row corrects the filing, the existing class rule then matches the entry, and no`);
p(`   \`ACTA_MATCH_OVERRIDES\` row was needed.`);
