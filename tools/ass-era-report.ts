...
/** The era pope's own briefs shelf, counted from the registry: what the brevia can match against. */
const eraIssuers = [...new Set(entries.map((e) => POPE_ISSUERS[e.pope]).filter((x): x is string => x !== undefined))];
const briefsShelf = docs.filter((d) => eraIssuers.includes(d.issuerId) && d.characteristics?.includes('in-forma-brevis')).length;
...
p(`4. **Every hold is \`series-not-created\`${breviaHeld === 0 ? '' : ', and the largest single group is the brevia'}.** The creator holds all ${creation.held.length} unmatched entries (spec decision 1: the ASS is joined, never created).${breviaHeld === 0 ? '' : ` **${breviaHeld} of the ${creation.held.length} holds are briefs**, from ${breviaVolumes.map((v) => `ASS ${v}`).join(', ')}. `
