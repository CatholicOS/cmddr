...
describe('the ASS reference (ass volumes spec, phase 2c-i: the sample)', () => {
  ...
  it('pins the matched count per volume, so a silent drop fails loudly', () => {
    ...
    expect(perVolume).toEqual({
      ...
      'ass-1': 0, 'ass-12': 5, 'ass-13': 2, 'ass-14': 0, 'ass-15': 0, 'ass-16': 2, 'ass-17': 0,
    ...
    expect(Object.fromEntries([...byIssuer].sort())).toEqual({ 'rp:leo-xiii': 117, 'rp:pius-x': 140 });
    ...
    expect(Object.fromEntries([...byGenre].sort())).toEqual({
      'apostolic-exhortation': 1, 'apostolic-letter': 38, encyclical: 41, letter: 174, 'papal-bull': 3,
    });
  });
  ...
  it('creates nothing from the ASS: no document carries an ass/ shelf, and every unmatched ASS entry is held series-not-created', () => {
    ...
    // created.
    expect(creation.held.filter((h) => h.entry.series === 'ASS' && h.reason === 'series-not-created')).toHaveLength(144);
    ...
    expect(Object.fromEntries([...reasons].sort())).toEqual({ 'series-not-created': 144 });
    expect(creation.held).toHaveLength(144);
  });
